...
| T21a | GitHub skill-link fallback | A JourneyMem GitHub URL or manual clone does not trigger repo exploration or `memory new`; the Agent treats it as a skill link and asks the two-choice first-use question after install/activation. | `scripts/public_release_check.py` |
